...
**Measured on reference-project** (offline, warm `poms-cache`, ~7 s):
...
### Concrete example (real, from the `reference-project` project — Spring/JSF, 25 modules)
...
2. **No regression.** Baseline on `reference-project`: **156 of 210 snyk findings covered, 46
...
- **Result on reference-project: identical (156 covered, 0 recovered).** The reference-project pins live in an
...
- **Result on reference-project: SEVERE REGRESSION — 156 → 65 covered, fad ids 240 → 149.**
...
Whatever the approach: the acceptance test is **covered ≥ 163 on reference-project AND ≥ baseline
...
# reference-project is the richer multi-module case below.
reference-project="/mnt/wsl/WipDrive/EY/sources-vote-reference-project-project-17.4-MINJU-1.2.0"
node fad-checker.js -s "$reference-project" --no-all-libs --no-retire --report-json /tmp/fad.json

# Snyk reference set already captured at /tmp/snyk-reference-project.json (snyk --all-projects --json
...
- Baseline snyk reference: `/tmp/snyk-reference-project.json` (regenerate via the cleaned `-t` tree + `snyk test --all-projects --json`).
