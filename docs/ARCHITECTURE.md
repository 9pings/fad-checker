...
2b. **Per-module version mediation** (`lib/version-overlay.js`, runs when step 2 does) — the global pass above applies any module's depMgmt pin across the WHOLE reactor, masking a *different* module's older (often vulnerable) transitive of the same coord. `expandPerModuleOverlay()` re-resolves **each module independently** with ONLY its own effective depMgmt (local parent chain via `core.resolveParentPath` + external parent/import-BOMs via `effectivePom`, `effCache`-memoised) and **appends** any genuinely-present version not already in `versions[]` (with `maskedVersions[]` provenance). Purely additive — never removes, never reseeds, so it can only ADD coverage. On melino: 156 → 181 Snyk-covered, 0 over-attribution FPs.
   ...
   **Attribution** — `attributeMaskedMatches(matches)` runs in `fad-checker.js` after every match source is merged (CVE index + OSV + snyk) and before anything reads `dep.scope`. A match on an overlay-recovered version is a transitive of ONE module, but `matchOne`'s per-version dep clone inherits the shared record's direct scope/pomPaths — so on a multi-project audit root it renders as a direct dep of the pom that pins the FIXED version and tops the exec summary's "direct production dependencies". Each match on a `maskedVersions[]` version is re-stamped (clone, never mutate) with `scope:"transitive"` + that entry's `via`/`viaPaths`/`depth` + the resolving module as `pomPaths`/`manifestPaths`. Locked by `test/overlay-attribution.test.js`.
