...
- **Air-gapped**; **zero network under `--offline`** (regression-tested), offline Maven transitive resolution, and `--osv-db` for cache-independent offline OSV recall. On **Apache Dubbo 2.7.8** (105 modules), running under `unshare -rn` with no network interface at all, fad recovers **657/657 (100%)** of OSV-Scanner's own online finding set, versus **37/657 (5.6%)** for OSV-Scanner offline, **45** for Grype+Syft and **40** for Trivy; reproduce it → [Benchmark](docs/BENCHMARK.md) · [Air-gapped](#air-gapped-audits)
