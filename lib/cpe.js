...
function refineMatchesWithCpe(matches, opts = {}) {
	...
	for (const m of matches) {
		...
		// The CPE coord-matcher (cpeMatchesDep) only maps maven + npm coordinates to NVD
		// CPEs. For other ecosystems (pypi/composer/nuget/go/ruby) a CPE can NEVER match,
		// so `affected:false` means "couldn't evaluate", NOT "false positive" — and OSV has
		// already done a precise, ecosystem-native version-range match. Filtering them here
		// would silently hide real CVEs (e.g. a vulnerable PyPI dep). Only let NVD CPE
		// contradict the match for coord types it actually understands.
		const cpeEvaluable = m.dep?.ecosystem === "maven" || m.dep?.ecosystem === "npm";
		if (!affected && rec.configurations.length && cpeEvaluable) {
