...
function abortSignal(timeoutMs) {
	if (!timeoutMs || !globalThis.AbortSignal?.timeout) return undefined;
...
async function tryRepos(repos, pathSuffix, opts = {}) {
	...
	for (const repo of repos) {
		...
		let r;
		try {
			r = await fetcher(url, { method, headers, signal: abortSignal(timeoutMs) });
		} catch (err) {
...
async function reachableRepos(repos, opts = {}) {
	...
	const checks = (repos || []).map(async repo => {
		...
		if (ah) headers.Authorization = ah;
		try {
			await fetcher(repo.url, { method: "HEAD", headers, signal: abortSignal(timeoutMs) });
			return repo;
		} catch {
			return null;
		}
