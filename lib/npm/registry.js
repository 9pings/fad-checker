...
async function checkNpmRegistryDeps(resolvedDeps, opts = {}) {
	...
	const printProgress = (final = false) => {
		...
		const pct = Math.round((processed / npmDeps.length) * 100);
		const line = `   npm registry: ${processed}/${npmDeps.length} (${pct}%) — ${elapsed}s`;
	...
	await Promise.all(npmDeps.map(dep => limit(async () => {
		...
		processed++;
		if (processed % 25 === 0 || processed === npmDeps.length) printProgress();
