 ...
 *        --export-csaf output; products mapped back to coords via their purl).
 *
...
function parseVex(csaf) {
	...
	for (const v of csaf.vulnerabilities || []) {
		...
		for (const pid of cleared) {
			...
			rules.push({ cve, coord: purl || null, coordRe: purl ? globToRe(purl) : null, reason: "VEX: not affected / fixed" });
		}
	}
	return rules;
}
