...
async function getAllInheritedProps(pomPath, allPomMetadata, cache) {
	...
	// Applied LAST so the current POM's own values win — even over an imported
	// BOM's project.* — letting an intra-reactor dep declared as
	// "<version>${project.version}</version>" resolve to THIS module's version.
