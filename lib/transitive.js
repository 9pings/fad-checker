			...
			for (const dep of eff.deps) {
				...
				if (visited.has(childKey)) {
					...
					if (existing) {
						...
						// WIDEST scope wins across paths. We dedupe by `g:a` and keep the first
						// chain walked, but scope is a property of the coord, not of the chain:
						// a coord reachable from both a test root and a compile root really is on
						// the compile classpath. Without this, BFS order decides, and a
						// production dependency that happens to be reached through a test path
						// first gets stamped "test" — dropping it out of the production count and
						// out of the --fail-on gate. Never narrow, only widen.
						if (existing.scope === "test" && propagated !== "test") existing.scope = propagated;
					}
					continue;
				}
