...
function directVsTransitive(prodMatches, t = x => x) {
 ...
 */
function mostVulnerableComponents(prodMatches, moduleNames, { topN = 7 } = {}) {
	const by = new Map();
	for (const m of prodMatches || []) {
		const sev = sevKey(m);
...
function fixPriority(prodMatches, t = x => x) {
...
function chartCard({ id, title, body, width, height, interactive, t = x => x }) {
...
function renderCharts(payload = {}, opts = {}) {
	...
	const components = multiModule ? mostVulnerableComponents(prod, moduleNames) : [];
