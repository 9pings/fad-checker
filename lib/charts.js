 ...
 * root" bar; (3) on a multi-module tree, the project's OWN modules carrying the most
 * critical/high vulns — donut, falling back to direct vs transitive on a single-descriptor
 * scan; (4) fix-priority bands — donut.
 *
...
function renderCharts(payload = {}, opts = {}) {
	...
	const cwe = cweByCriticality(prod, { t, cweTitle });
	const subs = vulnSubdepsByDep(prod, { t });
	...
	if (!(cwe.length || subs.length || scope.length || prio.length || unattr || components.length)) return "";
	...
	const note2 = unattr ? t("+{n} transitive CVE(s) with no resolved root", { n: unattr }) : "";
	...
	const W = Math.max(250, legendBoxWidth(cweSlices), legendBoxWidth(subSlices, note2), legendBoxWidth(multiModule ? components : scope), legendBoxWidth(prio));

	const card1 = cwe.length
		? chartCard({ t, id: "chart-cwe", title: t("CWE — direct vulns (by criticality)"), interactive, ...donutChart({ slices: cweSlices, width: W }) })
		: emptyCard("chart-cwe", t("CWE — direct vulns (by criticality)"), t("No categorised direct CVE."));

	const card2 = (subs.length || unattr)
		? chartCard({ t, id: "chart-subdeps", title: t("Sub-dep CVEs per dependency"), interactive, ...donutChart({ slices: subSlices, width: W, note: note2 }) })
		: emptyCard("chart-subdeps", t("Sub-dep CVEs per dependency"), t("No vulnerable transitive deps."));

	const card3 = multiModule
		? (components.length
