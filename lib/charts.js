...
function renderCharts(payload = {}, opts = {}) {
	...
	const card1 = cwe.length
		? chartCard({ id: "chart-cwe", title: t("CWE — direct vulns (by criticality)"), interactive, ...donutChart({ slices: cweSlices, width: W }) })
		: emptyCard("chart-cwe", t("CWE — direct vulns (by criticality)"), t("No categorised direct CVE."));

	const card2 = (subs.length || unattr)
		? chartCard({ id: "chart-subdeps", title: t("Sub-dep CVEs per dependency"), interactive, ...donutChart({ slices: subSlices, width: W, note: note2 }) })
		: emptyCard("chart-subdeps", t("Sub-dep CVEs per dependency"), t("No vulnerable transitive deps."));

	const card3 = multiModule
		? (components.length
			? chartCard({ id: "chart-components", title: t("Most vulnerable components"), interactive, ...donutChart({ slices: components, width: W }) })
			: emptyCard("chart-components", t("Most vulnerable components"), t("No critical or high production CVE.")))
		: (scope.length
			? chartCard({ id: "chart-scope", title: t("Direct vs transitive (by severity)"), interactive, ...donutChart({ slices: scope, width: W }) })
			: emptyCard("chart-scope", t("Direct vs transitive (by severity)"), t("No production CVE.")));

	const card4 = prio.length
		? chartCard({ id: "chart-priority", title: t("Fix priority"), interactive, ...donutChart({ slices: prio, width: W }) })
