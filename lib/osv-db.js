 ...
 *
 * Java is the one ecosystem where the per-dependency OSV cache is not a sound basis for
 * an audit: offline recall then depends on which coordinates happened to be queried
 * online at some earlier point, so a cold dep silently contributes nothing. A Maven or
 * Gradle scan therefore imports the full OSV Maven export on its FIRST run (~9 MB) and
 * loads the cached index on every later one, which makes offline recall complete and
 * cache-independent — the property `--offline` is sold on.
 *
 * Tri-state, courtesy of commander: `undefined` = decide here, `true` = the user asked
 * for it by name, `false` = `--no-osv-db`.
 *
 * @param {{osvDb?: boolean, osv?: boolean}} options
 * @param {{runMaven?: boolean, runGradle?: boolean}} eco
 */
function autoEnableOsvDb(options = {}, eco = {}) {
	...
	// --offline is a zero-network guarantee, so it must never trigger the import. An index
	// that is already on disk (warmed earlier, or carried in by --import-cache) costs
	// nothing to match against, and skipping it would lose recall precisely where the
	// local database earns its keep.
	if (options.offline) return !!eco.hasIndex;
	return true;
}
