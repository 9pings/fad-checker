...
async function queryPackagistAudit(resolvedDeps, opts = {}) {
	const { verbose, offline, fetcher = globalThis.fetch, cacheDir = DEFAULT_CACHE_DIR, onProgress, onSkipped } = opts;
	...
	if (missing.length && !offline) {
		...
		for (let i = 0; i < batches.length; i++) {
			...
			try {
				const res = await fetcher(url, { headers: { "User-Agent": "fad-checker-packagist-audit", Accept: "application/json" } });
				...
				// Packagist documents an omitted package as "no data". Only an explicit
				// empty array proves that a known package has no listed advisories.
				for (const name of batches[i]) {
					if (!byName.has(name) || !Array.isArray(byName.get(name)))
						throw new Error(`Packagist audit has no advisory data for ${name}`);
