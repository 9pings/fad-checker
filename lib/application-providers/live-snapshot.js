...
async function responseJson(response, label) {
	...
	return { body, sha256: crypto.createHash("sha256").update(text).digest("hex") };
}
...
async function fetchDrupalAdvisories(packages = [], { fetchImpl = globalThis.fetch, apiUrl = DRUPAL_ADVISORIES_URL, now = Date.now() } = {}) {
	...
	const { body, sha256 } = await responseJson(await require("../providers").getResource("drupal", "advisories", { packages: queried },
...
function mergeDrupalSnapshots(cached, fetched) {
	...
	return { snapshot, sourceSnapshot: { ...fetched.sourceSnapshot,
...
async function fetchWordfenceFeed(url, { fetchImpl = globalThis.fetch, now = Date.now(), apiKey } = {}) {
	if (!apiKey || typeof apiKey !== "string" || !apiKey.trim())
		throw new Error("Wordfence live scan requires an API key (--wordfence-api-key or WORDFENCE_API_KEY)");
	const sourceUrl = String(url);
	const { body, sha256 } = await responseJson(await require("../providers").getResource("wordfence", "production-feed", {}, {
		fetcher: fetchImpl, signal: AbortSignal.timeout(120000), headers: { Authorization: `Bearer ${apiKey.trim()}` },
...
async function fetchGithubAdvisories(repo, { fetchImpl = globalThis.fetch, apiUrl = GITHUB_ADVISORIES_URL,
	...
	for (let page = 1; page <= maxPages; page++) {
		...
		const { body, sha256 } = await responseJson(response, "Github security-advisories");
...
async function fetchWordpressChecksums(version, locale = "en_US", { fetchImpl = globalThis.fetch,
	...
	const { body, sha256 } = await responseJson(await require("../providers").getResource("wordpress", "checksums",
