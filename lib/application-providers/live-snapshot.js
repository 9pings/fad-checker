...
async function fetchDrupalAdvisories(packages = [], { fetchImpl = globalThis.fetch, apiUrl = DRUPAL_ADVISORIES_URL, now = Date.now() } = {}) {
	...
	const { body, sha256 } = await responseJson(await fetchImpl(url.toString(), { signal: AbortSignal.timeout(60000) }),
...
async function fetchWordfenceFeed(url, { fetchImpl = globalThis.fetch, now = Date.now(), apiKey } = {}) {
	...
	const { body, sha256 } = await responseJson(await fetchImpl(sourceUrl, {
		signal: AbortSignal.timeout(120000), headers: { Authorization: `Bearer ${apiKey.trim()}` },
	}), "Wordfence v3 feed");
	const collectedAt = new Date(now).toISOString();
...
async function fetchWordpressChecksums(version, locale = "en_US", { fetchImpl = globalThis.fetch,
	...
	const { body, sha256 } = await responseJson(await fetchImpl(url, { signal: AbortSignal.timeout(60000) }),
		"WordPress checksums");
	const collectedAt = new Date(now).toISOString();
