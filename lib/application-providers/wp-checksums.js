...
function evaluateChecksums(reference, fileAccess, { maxDiagnosticsPerCode = DEFAULT_MAX_DIAGNOSTICS_PER_CODE } = {}) {
	...
	const emit = (code, files, verb) => {
		for (const file of files.slice(0, maxDiagnosticsPerCode))
			diagnostics.push({ code, path: file,
				message: `${file} ${verb} the official WordPress ${reference.version || ""} distribution reference`.replace(/ +/g, " ").trim() });
		if (files.length > maxDiagnosticsPerCode)
			diagnostics.push({ code: "CMS_INTEGRITY_LIST_TRUNCATED",
				message: `${files.length} file(s) ${verb} the distribution reference; the report lists the first ${maxDiagnosticsPerCode}` });
	};
	emit("CMS_FILE_MODIFIED", modified, "differs from");
	emit("CMS_FILE_MISSING", missing, "is part of the official distribution but absent from the scanned tree");
	emit("CMS_FILE_EXTRA", extra, "is not part of");
