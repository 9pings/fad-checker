...
module.exports = {
	...
	async assessChecksums(app, components, ctx) {
		...
		if (!ctx.wpChecksumsPath && !ctx.liveWpChecksumsUrl) return notRun("CMS_PROVIDER_UNCONFIGURED");
		...
		let cached;
		if (ctx.wpChecksumsPath) {
			const key = `wp-checksums:${ctx.wpChecksumsPath}`;
			cached = ctx.snapshotCache.get(key);
			if (!cached) {
				const validated = ctx.validatedLocalSources?.get(ctx.wpChecksumsPath);
				const stat = validated?.stat || fs.statSync(ctx.wpChecksumsPath);
				...
				const bytes = validated?.bytes || fs.readFileSync(ctx.wpChecksumsPath);
	...
	async assess(app, components, ctx) {
		...
		if (!cached && ctx.wordfenceFeedPath) {
			...
			const declared = declaredCollectedAt(feed);
			const sourceSnapshot = { sha256: crypto.createHash("sha256").update(bytes).digest("hex"),
				fileModifiedAt: stat.mtime.toISOString(), completeness: "operator-declared",
				...(declared ? { collectedAt: declared.toISOString() } : {}) };
