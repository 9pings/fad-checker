...
async function runApplicationPlugins(root, opts = {}) {
	...
	validateLocalSource(opts.wordfenceFeedPath, "Wordfence", feed => require("../application-providers/wordfence-v3").indexFeed(feed));
	validateLocalSource(opts.drupalAdvisoriesPath, "Drupal advisories", require("../application-providers/drupal-advisories").validateSnapshot);
	validateLocalSource(opts.prestashopAdvisoriesPath, "PrestaShop advisories",
		feed => require("../application-providers/github-advisories").validateSnapshot(feed, { fallbackCoord: "prestashop/prestashop" }));
	validateLocalSource(opts.typo3AdvisoriesPath, "TYPO3 advisories",
	...
	const context = {
		...
		wordfenceFeedPath: opts.wordfenceFeedPath || null,
		drupalAdvisoriesPath: opts.drupalAdvisoriesPath || null,
		...
		prestashopAdvisoriesPath: opts.prestashopAdvisoriesPath || null,
		livePrestashopAdvisoriesUrl: opts.livePrestashopAdvisoriesUrl || null,
		typo3AdvisoriesPath: opts.typo3AdvisoriesPath || null,
		...
		for (const candidate of candidates || []) {
			...
			try {
				...
				if (Array.isArray(assessed)) result.findings.push(...assessed);
					...
					for (const check of assessed.coverage || []) journal.record(check);
				}
			} catch (error) {
