...
const VERDICT = {
	// The public record has no such advisory: a vendor-proprietary id. Unreachable by
	// any amount of matching work — it is original research, not a mapping problem.
	NO_PUBLIC_RECORD: "NO_PUBLIC_RECORD",
...
function classifyPair(pair, osvRecord) {
	const base = { coord: pair.coord, version: pair.version, id: osvRecord ? osvRecord.id : (pair.id || null), boundCoords: [] };
	if (!osvRecord) return { ...base, verdict: VERDICT.NO_PUBLIC_RECORD };
...
module.exports = { classifyPair, summarize, mergeRecords, VERDICT, REAL_GAP };
