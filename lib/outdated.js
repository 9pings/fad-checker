...
async function checkEolDeps(resolvedDeps, opts = {}) {
	...
	const cache = injected ? { meta: { fetchedAt: 0 }, entries: {} } : loadJsonCache(EOL_CACHE_PATH);
	const fresh = !injected && isEolCacheFresh();
	const seen = new Set();
	...
	for (const dep of resolvedDeps.values()) {
		...
		let cycles;
		if (injected) cycles = injected[product.product];
		else cycles = fresh && cache.entries[product.product]
	...
	const results = plain.concat(groupFrameworkHits(frameworkHits));
	if (!injected) {
...
// Placeholder until Task 4 — framework hits are returned one-per-dep for now.
function groupFrameworkHits(hits) { return hits; }
