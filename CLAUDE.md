...
```bash
npm install
npm test                  # 497 unit tests via node --test
...
lib/codecs/gradle.codec.js   Gradle codec. Reads gradle.lockfile / libs.versions.toml / build.gradle(.kts) (incl. buildSrc/), emits ecosystem:"maven" + ecosystemType:"gradle" records, surfaces platform() BOMs in _gradle.platformBoms for the maven-bom backfill. gradle/parse.js (lockfile+properties+DSL) + gradle/catalog.js (version-catalog + accessor resolution).
...
lib/codecs/pypi/parse.js          poetry.lock/Pipfile.lock/uv.lock/pdm.lock (smol-toml) + pyproject.toml (PEP 621/poetry) + the whole requirements*.txt family (recursive -r/-c; requirements-pinned.txt etc. — pip-compile/Dependabot keep the real pins there; requirements-(dev|test|lint|docs|…) classed dev) parsers (PEP 503).
...
data/                        known-obsolete.json, eol-mapping.json, cpe-coord-map.json, known-public-namespaces.json, license-policy.json, popular-packages.json (typosquat targets)
...
- **Every distinct version is scanned, not just the highest**: when profiles/modules pin the same `g:a` to different versions, the resolved-dep entry keeps `version` = highest (representative for display/EOL/outdated) but `versions` = all distinct concrete versions. CVE matching (`matchOne`) and OSV (`queryOsvForDeps`) iterate `versions` so a vuln affecting only a lower-versioned profile variant isn't missed. Match dedup keys are `g:a:version|cve.id` (version included) to preserve per-version findings. **Masked transitive versions are added by `lib/version-overlay.js`** (see below): they land in `versions[]` with a `maskedVersions[]` provenance entry (`{version, via, module}`).
- **Per-module version mediation (`lib/version-overlay.js`)**: fad's global transitive pass (`expandWithTransitives`) resolves the WHOLE reactor as one tree with one global `rootDepMgmt` = the *highest* version of every coord. Maven instead applies a `<dependencyManagement>` pin only inside the **subtree that declares it**, so a pin in module A (e.g. `controller` pins `poi 5.4.1`) force-overrides the *same coord's* older transitive in an unrelated module B (e.g. the stress-tests island's `jmeter → poi 3.11`), hiding its CVE. The overlay runs AFTER the global pass (so it never regresses it) and re-resolves **each module independently** with ONLY that module's effective depMgmt — rebuilt by climbing its **local parent chain** (`core.resolveParentPath`) + its external `<parent>`/import-BOMs — then **appends** any genuinely-present version not already in `versions[]`. Purely additive: it only ever ADDS coverage, and because it uses real per-module mediation the version it surfaces is the one actually on that module's classpath (a true positive, not a force-elevated one). Offline-aware (cache-first) + `effCache`-memoised across modules. Gated by `--transitive`; measured on the 25-module `reference-project` project as **156 → 181 Snyk-covered findings, +0 version-contradictions**.
...
- **Anonymized descriptor (offline→online round-trip)**: `--export-anonymized <f>` (offline, needs `-s`) serializes the collected `resolved` Map to a flat `fad-deps/1` JSON keeping only public coordinates (`ecosystem`/`ecosystemType`/`namespace`/`name`/`version`/`versions`/`scope`/`isDev`) — **strips** paths, registry URLs, integrity, parent chains — then exits. `--import-anonymized <f>` (online, **no `-s`**) rebuilds the Map (empty `manifestPaths`, recomputed `coordKey`) via `lib/deps-descriptor.js` and runs `runReportFlow` purely to **warm the coordinate-keyed caches** — it **writes NO report by default** (it's a cache-warming step, not a reporting one: `options.report` is forced off in the import branch unless an explicit `--report-<type>` is given). Round-trip = export (offline) → import + `--export-cache` (online) → `--import-cache` + normal `--offline -s ./proj` (offline) for the full path-bearing report. Private/public sorting is the auditor's job via `-e` (offline can't check Maven Central). If a report IS explicitly requested in import mode it stays path-free (`projectInfo.src` = withheld).

## Testing

```bash
node --test test/*.test.js            # full suite (497 tests)
...
- `gradle-kotlin-catalog/` — Kotlin DSL + `libs.versions.toml` + `buildSrc/` convention plugin with a `platform()` BOM (promoliv-batch shape); `gradle-groovy-lockfile/` — Groovy DSL + `gradle.lockfile` (authoritative); `gradle-hybrid/` — `pom.xml` + `build.gradle` coexisting (keyspace non-collision)
...
- **retire.js signatures live in `~/.fad-checker/retire-signatures/jsrepository-v5.json`** (not retire's default `/tmp/.retire-cache`, whose 1 h TTL would force a network refetch). We pass them via `--jsrepo <file>` so retire loads from disk — **no network, no TTL**. `warmRetireSignatures()` fetches them online; `--export-cache` bundles them so phase-3 offline JS scanning works. retire is **offline-only** in the air-gapped flow (it needs the actual `.js` files, absent online) and its findings cache stays path-keyed (`md5(srcDir)`), which is fine since it only runs on the same offline machine/path. With no source dir (`--import-anonymized`), `runRetire` returns `null` (nothing to scan).
